...
if (!code) {
  // on first login - if there is no code object
  document
  ...
  if (sessionStorage.getItem("recommended")) {
    ...
    recommended = await fetchRecommended(
      accessToken,
      JSON.parse(sessionStorage.getItem("tracksShort"))
    );
...
async function fetchRecommended(token, tracks) {
  ...
  var recTracks = [];
  recIndices.forEach((i) => recTracks.push(tracks.items[i]));
...
async function toPlaylist(tracksUri, name, desc) {
  ...
  const playlist = await fetchWebApi(
    `v1/users/${window.profile.id}/playlists`,
    "POST",
    {
      name: name,
      description: desc,
...
function populateUI(profile, tracks) {
  ...
  const displayName = profile.display_name.replaceAll(/\p{Emoji}/gu, "");
  document.getElementById("cassette-title").innerText =
    displayName.toLowerCase() + titleSuffix;
  document.getElementById("mixtape-name-input").value =
    displayName.toLowerCase() + titleSuffix;
  ...
  for (var i = 0; i < 20; i++) {
    ...
    document.getElementById("track-" + (i + 1)).innerHTML =
...
document
  .getElementById("create-playlist-mobile")
  .addEventListener("click", createPlaylist);
function createPlaylist() {
  ...
  const tracksDict = {
    "short-term": window.tracksShort.items,
    "medium-term": window.tracksMedium.items,
    "long-term": window.tracksLong.items,
    recommend: window.recommended.tracks,
  };
  ...
  const playlist = toPlaylist(
    trackList,
    document.getElementById("mixtape-name-input").value,
    `${timeTags[window.timeRange]} Generated on ${
      document.getElementById("date").innerHTML
    } by mixedify. Get yours at www.mixedify.netlify.app!`
  );
  playlist.then((pl) => {
    console.log(pl);
    setTimeout(() => {
    ...
    document.getElementById("create-playlist-mobile").innerHTML =
      "Create playlist";
  });
}
...
async function updateRecs() {
  ...
  const newRecommended = await fetchRecommended(
    accessToken,
    JSON.parse(sessionStorage.getItem("tracksShort"))
  );
...
document.getElementById("refresh-rec").addEventListener("click", function () {
  ...
  if (timeRange == "recommend") {
    ...
    const populateRecs = async () => {
      ...
      const displayName = window.profile.display_name.replaceAll(
        /\p{Emoji}/gu,
        ""
      );
      document.getElementById("cassette-title").innerText =
        "recs for " + displayName.toLowerCase();
      document.getElementById("mixtape-name-input").value =
        "recs for " + displayName.toLowerCase();
...
document.querySelectorAll(".time-range-option").forEach((btn) => {
  ...
  btn.addEventListener("click", function () {
    if (!this.classList.contains("active")) {
      ...
      const tracksDict = {
        "short-term": window.tracksShort,
        "medium-term": window.tracksMedium,
        "long-term": window.tracksLong,
        recommend: window.recommended,
      };
      const timeRangeDict = {
        "short-term": "Last month",
        "medium-term": "Last 6 months",
        "long-term": "Last 12 months",
        recommend: "Recommended for " + displayName.toLowerCase(),
      };
...
document.getElementById("short-term").addEventListener("click", function () {
  const displayName = profile.display_name.replaceAll(/\p{Emoji}/gu, "");
  document.getElementById("cassette-title").innerText =
    displayName.toLowerCase() + "'s past month";
  document.getElementById("mixtape-name-input").value =
    displayName.toLowerCase() + "'s past month";
});
...
document.getElementById("medium-term").addEventListener("click", function () {
  const displayName = profile.display_name.replaceAll(/\p{Emoji}/gu, "");
  document.getElementById("cassette-title").innerText =
    displayName.toLowerCase() + "'s mix vol. 1";
  document.getElementById("mixtape-name-input").value =
    displayName.toLowerCase() + "'s mix vol. 1";
});
...
document.getElementById("long-term").addEventListener("click", function () {
  const displayName = profile.display_name.replaceAll(/\p{Emoji}/gu, "");
  document.getElementById("cassette-title").innerText =
    displayName.toLowerCase() + "'s past year";
  document.getElementById("mixtape-name-input").value =
    displayName.toLowerCase() + "'s past year";
});
...
document
  ...
  .addEventListener("click", console.log("Downloading"));
